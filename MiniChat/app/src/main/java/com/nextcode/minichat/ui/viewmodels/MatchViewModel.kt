...
import com.nextcode.minichat.domain.repositories.MatchRepository
...
class MatchViewModel @Inject constructor(
    ...
    private val userRepository: com.nextcode.minichat.domain.repositories.UserRepository,
    private val tokenManager: com.nextcode.minichat.data.local.TokenManager,
    private val apiService: com.nextcode.minichat.data.remote.ApiService,
    @dagger.hilt.android.qualifiers.ApplicationContext private val context: android.content.Context
) : ViewModel() {

    private val _currentUser = MutableStateFlow<com.nextcode.minichat.data.User?>(null)
    val currentUser = _currentUser.asStateFlow()

    private val _selectedCountry = MutableStateFlow(
        com.nextcode.minichat.data.Country("AUTO", R.string.all_countries, "🌍")
    )
    ...
    val countries = listOf(
        com.nextcode.minichat.data.Country("AUTO", R.string.all_countries, "🌍"),
        com.nextcode.minichat.data.Country("US", R.string.country_us, "🇺🇸"),
        com.nextcode.minichat.data.Country("GB", R.string.country_gb, "🇬🇧")
    ) + com.nextcode.minichat.data.StaticData.getCountries().filter { it.code != "US" && it.code != "GB" }
    ...
    init {
        ...
        viewModelScope.launch {
            ...
            userRepository.currentUser.collect { userEntity ->
                if (userEntity != null) {
                    _currentUser.value = com.nextcode.minichat.data.User(
                        id = userEntity.id,
                        username = userEntity.username,
                        email = userEntity.email,
                        karma = userEntity.karma,
                        diamonds = userEntity.diamonds,
                        country = com.nextcode.minichat.data.Country(userEntity.countryCode, userEntity.countryNameRes, userEntity.countryFlag),
    ...
    fun updateCountry(country: com.nextcode.minichat.data.Country) {
    ...
    private fun listenForEvents() {
        viewModelScope.launch {
            repository.matchEvents.collect { event ->
                when (event) {
                    is com.nextcode.minichat.data.remote.WebSocketEvent.Connected -> {
                        // Success connect
                    }
                    is com.nextcode.minichat.data.remote.WebSocketEvent.MatchFound -> {
                        matchingTimeoutJob?.cancel()
                        handleMatchFound(event.data)
                    }
                    is com.nextcode.minichat.data.remote.WebSocketEvent.Signal -> {
                        handleSignal(event.type, event.data)
                    }
                    is com.nextcode.minichat.data.remote.WebSocketEvent.ChatMessage -> {
                    ...
                    is com.nextcode.minichat.data.remote.WebSocketEvent.Error -> {
    ...
    private fun handleMatchFound(data: String) {
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                val partnerUser = com.nextcode.minichat.data.User(
                    id = partnerId, // EXPECTS STRING
                    username = partnerUsername,
                    email = "", // Placeholder
                    karma = 0,
                    diamonds = 0,
                    country = com.nextcode.minichat.data.Country("Unknown", R.string.all_countries, "❓"),
                    isPremium = false
                )
