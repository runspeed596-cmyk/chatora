...
class MatchService(
    ...
    /**
     * Data class for exposing active matches to admin panel.
     * Purely in-memory/ephemeral — no database storage.
     */
    ...
    /**
     * Returns all currently active video chat matches.
     * Data is ephemeral — it exists only while the match is live.
     * When a user presses "findNext" or disconnects, the match is immediately removed.
     */
    fun getActiveMatches(): List<ActiveMatchInfo> {
        val seen = mutableSetOf<String>()
        return activeMatches.entries.mapNotNull { (username, partnerUsername) ->
            val matchId = userMatchIds[username] ?: return@mapNotNull null
            // Deduplicate: each match appears for both users, only return once
            if (seen.contains(matchId)) return@mapNotNull null
            seen.add(matchId)
            ActiveMatchInfo(
                matchId = matchId,
                user1 = username,
